...
class ImageLIst extends Component {
    state = { 
     ...
     getImages = () => {
         axios.get('http://127.0.0.1:8000/api/images/', { headers: {'accept': 'application/json'}})
         .then(resp => {
             this.setState({
                 images: resp.data,
                 status: true,
    ...
    render() { 
        const images = this.state.images.slice(0, this.state.visible)
        .map(img => {
        ...
        return ( 
            <div>
                { this.state.isLoading ?
                    ...
                    <React.Fragment>
                        { ((this.state.images.length === 0) && (this.state.status)) && <h3>No images classified</h3> }
                        {images}
                        { this.state.newLoaded && <Spinner animation="border" role="status"></Spinner> }
                        <br />
